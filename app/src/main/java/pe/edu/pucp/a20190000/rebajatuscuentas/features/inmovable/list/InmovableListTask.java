...
public class InmovableListTask extends AsyncTask<Void, Void, List<InmovableMainData>> {
    ...
    private WeakReference<InmovableListActivity> view;

    public InmovableListTask(InmovableListActivity view) {
    ...
    protected List<InmovableMainData> doInBackground(Void... voids) {
        ...
        InmovableListActivity view = this.view.get();
        if (view == null) return null;
        // Inicializar la base de datos, si es que aún no se hizo
        AppDatabase database = AppDatabase.getInstance(view);
        if (database == null) {
